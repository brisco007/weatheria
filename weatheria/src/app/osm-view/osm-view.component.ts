...
export class OsmViewComponent implements OnInit, OnDestroy {
  ...
  geoSearchService =
    "https://nominatim.openstreetmap.org/search/{ville}?format=json&addressdetails=1&limit=1&polygon_svg=1";
  @Input()
  width: string;
  @Input()
  height: string;
  ...
  searchGeo(town: string) {
    const service = (this.geoSearchService || "").replace(
      new RegExp("{ville}", "ig"),
      `${town}`
    );

    this.searchGeoSub = this.httpClient.get(service).subscribe(data => {
      const val = data[0] || {};
      console.log(val);
      ...
      this.latitudePointer = val["lat"];
      this.longitudePointer = val["lon"];
      this.addressSelected.emit(this.address);
      this.addressChanged.emit(this.pointedAddress);
    });
  }
